					...
					<div class="board-wrap">
                        ...
                        <div class="board-add-header" style="border-bottom:1px solid #ddd;">
							...
							<ul>
								<li class="li01">이름</li>
								<li class="li05">
									<p class="con-txt"><input type="text" v-model="programApplication.name"></p>
								</li>
						...
						<div class="board-btn-list">
                            <button type="button" class="btn" @click="goApplication">신청</button>
                            <router-link role="button" class="btn li02" :to="`/admin/program/${this.program.id}`">취소</router-link>
						</div>
						<!-- //버튼 -->
						<!-- //컨텐트 -->
					</div>
                ...
                this.$http.post(`${this.store.getters.restWebPath}/programApp` ,this.programApplication)
                    .then(({data})=>{
                        if(data.result == "success"){
                            alert(data.msg);
                            this.router.push(`/admin/program/${this.id}`);
                        }else{
                            alert(data.msg);
                        }
                    })
